...
static nbt_tag_t *nbt__parse(
    nbt__read_stream_t *stream, int parse_name, nbt_tag_type_t override_type
)
{
    ...
    case NBT_TYPE_COMPOUND: {
        ...
        for (;;) {
            ...
            if (inner_tag->type == NBT_TYPE_END) {
                ...
                tag->tag_compound.value = (nbt_tag_t **) NBT_REALLOC(
                    tag->tag_compound.value, (tag->tag_compound.size + 1) * sizeof(nbt_tag_t *)
                );
    ...
    case NBT_TYPE_INT_ARRAY: {
        ...
        tag->tag_int_array.value = (int32_t *) NBT_MALLOC(
            tag->tag_int_array.size * sizeof(int32_t)
        );
    ...
    case NBT_TYPE_LONG_ARRAY: {
        ...
        tag->tag_long_array.value = (int64_t *) NBT_MALLOC(
            tag->tag_long_array.size * sizeof(int64_t)
        );
...
nbt_tag_t *nbt_parse(nbt_reader_t reader, int parse_flags)
{
    ...
    if (compressed) {
        ...
        int ret = inflateInit2(
            &stream, gzip_format ? -Z_DEFAULT_WINDOW_BITS : Z_DEFAULT_WINDOW_BITS
        );
        if (ret != Z_OK) {
            NBT_FREE(buffer);
            return NULL;
...
void nbt_write(nbt_writer_t writer, nbt_tag_t *tag, int write_flags)
{
    ...
    if (compressed) {
        ...
        if (deflateInit2(
                &stream, NBT_COMPRESSION_LEVEL, Z_DEFLATED, window_bits, 8, Z_DEFAULT_STRATEGY
            )
            != Z_OK) {
...
void nbt_tag_list_append(nbt_tag_t *list, nbt_tag_t *value)
{
    list->tag_list.value = (nbt_tag_t **) NBT_REALLOC(
        list->tag_list.value, (list->tag_list.size + 1) * sizeof(nbt_tag_t *)
    );
...
void nbt_tag_compound_append(nbt_tag_t *compound, nbt_tag_t *value)
{
    compound->tag_compound.value = (nbt_tag_t **) NBT_REALLOC(
        compound->tag_compound.value, (compound->tag_compound.size + 1) * sizeof(nbt_tag_t *)
    );
...
nbt_tag_t *nbt_tag_compound_getidx(nbt_tag_t *tag, size_t index)
{
    return tag->tag_compound.value[index];
}
...
nbt_tag_t *nbt_copy_tag(const nbt_tag_t *tag)
{
    ...
    case NBT_TYPE_BYTE_ARRAY:
        ...
        NBT_MEMCPY(
            new_tag->tag_byte_array.value, tag->tag_byte_array.value, tag->tag_byte_array.size
        );
        break;
    case NBT_TYPE_STRING:
    ...
    case NBT_TYPE_LIST:
        ...
        new_tag->tag_list.value = (nbt_tag_t **) NBT_MALLOC(
            tag->tag_list.size * sizeof(nbt_tag_t *)
        );
        for (size_t i = 0; i < tag->tag_list.size; i++) {
    ...
    case NBT_TYPE_COMPOUND:
        ...
        new_tag->tag_compound.value = (nbt_tag_t **) NBT_MALLOC(
            tag->tag_compound.size * sizeof(nbt_tag_t *)
        );
        for (size_t i = 0; i < tag->tag_compound.size; i++) {
    ...
    case NBT_TYPE_INT_ARRAY:
        ...
        new_tag->tag_int_array.value = (int32_t *) NBT_MALLOC(
            tag->tag_int_array.size * sizeof(int32_t)
        );
        NBT_MEMCPY(
            new_tag->tag_int_array.value, tag->tag_int_array.value,
            tag->tag_int_array.size * sizeof(int32_t)
        );
        break;
    case NBT_TYPE_LONG_ARRAY:
        new_tag->tag_long_array.size = tag->tag_long_array.size;
        new_tag->tag_long_array.value = (int64_t *) NBT_MALLOC(
            tag->tag_long_array.size * sizeof(int64_t)
        );
        NBT_MEMCPY(
            new_tag->tag_long_array.value, tag->tag_long_array.value,
            tag->tag_long_array.size * sizeof(int64_t)
        );
        break;
    default:
...
bool nbt_compare_tags(const nbt_tag_t *tag1, const nbt_tag_t *tag2)
{
    ...
    case NBT_TYPE_BYTE_ARRAY:
        ...
        return NBT_MEMCMP(
                   tag1->tag_byte_array.value, tag2->tag_byte_array.value, tag1->tag_byte_array.size
               )
            == 0;
    case NBT_TYPE_STRING:
        ...
        return NBT_MEMCMP(tag1->tag_string.value, tag2->tag_string.value, tag1->tag_string.size)
            == 0;
    case NBT_TYPE_LIST:
    ...
    case NBT_TYPE_INT_ARRAY:
        ...
        return NBT_MEMCMP(
                   tag1->tag_int_array.value, tag2->tag_int_array.value,
                   tag1->tag_int_array.size * sizeof(int32_t)
               )
            == 0;
    case NBT_TYPE_LONG_ARRAY:
        ...
        return NBT_MEMCMP(
                   tag1->tag_long_array.value, tag2->tag_long_array.value,
                   tag1->tag_long_array.size * sizeof(int64_t)
               )
            == 0;
    default:
        return false;
